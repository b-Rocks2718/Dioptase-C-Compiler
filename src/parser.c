...
// Build an abstract declarator from pointer, array, and function suffixes.
struct Type* process_abstract_declarator(
    struct AbstractDeclarator* declarator, 
    struct Type* base_type){
  struct Type* result;
...
// Parse an integer, character, or floating literal expression.
...
// Parse a string literal and decode its escape sequences.
...
// Parse postfix operators such as calls, subscripts, and member access.
...
// Parse a sizeof expression for either a type name or an expression.
struct Expr* parse_sizeof(){
  ...
  if ((expr = parse_sub_factor())){
    // sizeof (<expr>)
    struct Expr* sizeof_expr = alloc_expr(SIZEOF_EXPR, start->start);
  ...
  struct Expr* sizeof_t_expr = alloc_expr(SIZEOF_T_EXPR, start->start);
  sizeof_t_expr->expr.sizeof_t_expr.type = type;
  return sizeof_t_expr;
}

// Parse a parenthesized or otherwise grouped factor.
...
// Parse a primary expression followed by any postfix operations.
...
// Return whether the token sequence begins a declaration type specifier.
...
// Apply declarator syntax to a base type.
...
// Parse GNU-style variable attributes, including cleanup callbacks.
...
// Parse the fields of a struct or union until its closing brace.
...
// Parse enumerators and assign implicit values where needed.
