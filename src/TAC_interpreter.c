...
// Release a TacMemory's allocated resources.
...
// Grow the TacMemory cell array if needed.
...
// Find an existing memory cell by address.
...
// Ensure a memory cell exists for a given address.
...
// Allocate a contiguous range of memory slots.
...
// Release memory held by a TacBindings structure.
...
// Find an existing binding by name.
...
// Create or return a binding with a specific slot allocation.
...
// Create or return a binding for a name.
// Returns the binding for the identifier, allocating storage if missing.
...
// Release resources held by a CopyTo/FromOffset requirement map.
...
// Grow the CopyTo/FromOffset requirement map if needed.
...
// Find an existing CopyTo/FromOffset requirement by name.
...
// Release memory held by a TacFunctionTable.
...
// Find a function table entry by address.
// Returns the entry pointer or NULL if not found.
...
// Write a variable value by name, creating storage if needed.
...
// Release resources associated with a function frame.
...
// Execute supported built-in calls that have no TAC definition.
